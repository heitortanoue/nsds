...
//        final Dataset<Row> enrichedStream = inStream
//                .join(productsClassification, col("value").equalTo(col("product")))
//                .groupBy("classification")
//                .count();

        final Dataset<Row> enrichedStream = inStream
                .join(productsClassification, col("value").mod(10).equalTo(col("product")))
                .groupBy(window(col("timestamp"), "30 seconds", "5 seconds"), col("classification"))
                .count();

        final StreamingQuery query = enrichedStream
                .writeStream()
                .outputMode("update")
                .format("console")
                .start();
